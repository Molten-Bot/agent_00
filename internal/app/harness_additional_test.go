...
func TestRunRecreatesPRWhenNoChecksReportedOnMergedPR(t *testing.T) {
	t.Parallel()
	...
	newPRURL := "https://github.com/acme/repo/pull/43"
	...
	fake := &fakeRunner{t: t, exps: []expectedRun{
		...
		{cmd: branchMoveCommand(repoDir, renamedBranch)},
		{cmd: pushCommand(repoDir, renamedBranch)},
		{cmd: prCreateCommand(repoDir, cfg, renamedBranch), res: execx.Result{Stdout: newPRURL + "\n"}},
		{cmd: prChecksCommand(repoDir, newPRURL)},
	}}
	...
	if got, want := res.PRURL, newPRURL; got != want {
		t.Fatalf("PRURL = %q, want %q", got, want)
	}
	if len(fake.exps) != 0 {
		t.Fatalf("unconsumed expectations: %d", len(fake.exps))
