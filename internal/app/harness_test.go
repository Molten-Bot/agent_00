...
func TestAutoMergeCleanReviewSkipsGitHubMergePermissionDenied(t *testing.T) {
	t.Parallel()
	...
	if err := h.autoMergeCleanReview(context.Background(), repo, "42", "acme/repo", "squash", metadata, outcome, false); err != nil {
		t.Fatalf("autoMergeCleanReview() err = %v, want nil for GitHub permission-denied auto-merge", err)
	}
	if len(fake.exps) != 0 {
		t.Fatalf("unconsumed expectations: %d", len(fake.exps))
	}
