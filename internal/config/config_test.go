...
func TestLoadSupportsReviewTaskHeadBranchSelector(t *testing.T) {
	t.Parallel()
...
func TestApplyDefaultsAppendsAgent00FooterToDefaultPRBody(t *testing.T) {
	t.Parallel()
	...
	if !strings.Contains(cfg.PRBody, prBodyGeneratedByLine) {
		t.Fatalf("PRBody = %q, want Agent_00 header", cfg.PRBody)
	}
	if strings.Contains(cfg.PRBody, prBodyFormatReferenceLine) {
		t.Fatalf("PRBody = %q, want to omit deprecated PR format reference line", cfg.PRBody)
	}
	if !strings.Contains(cfg.PRBody, prBodyDefaultSummary) {
		t.Fatalf("PRBody = %q, want standardized summary text", cfg.PRBody)
	}
...
func TestApplyDefaultsAppendsAgent00FooterToCustomPRBody(t *testing.T) {
	t.Parallel()
	...
	if !strings.Contains(cfg.PRBody, prBodyDefaultSummary+"\n\nCustom PR body.") {
		t.Fatalf("PRBody = %q, want custom body preserved after standardized summary text", cfg.PRBody)
	}
	if !strings.Contains(cfg.PRBody, "Original task prompt:\n```text\nfix tests\n```") {
		t.Fatalf("PRBody = %q, want original prompt block", cfg.PRBody)
	}
...
func TestApplyDefaultsDoesNotDuplicateAgent00Footer(t *testing.T) {
	t.Parallel()
	...
	if got := strings.Count(cfg.PRBody, "https://molten.bot/code?source=pr"); got != 1 {
		t.Fatalf("PRBody contains %d CTA links, want exactly 1: %q", got, cfg.PRBody)
	}
}

func TestApplyDefaultsRemovesLegacyPRFormatReferenceLine(t *testing.T) {
	t.Parallel()
	...
	if strings.Contains(cfg.PRBody, "Agent summary of issue resolution:") {
		t.Fatalf("PRBody retained legacy summary heading: %q", cfg.PRBody)
	}
	if !strings.Contains(cfg.PRBody, prBodyDefaultSummary+"\n\nCustom PR body.") {
		t.Fatalf("PRBody = %q, want default summary followed by custom body", cfg.PRBody)
	}
}
